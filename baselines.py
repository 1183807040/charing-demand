...
import copy
from models import MultiHeadsGATLayer
...
class VAR(nn.Module):
    ...
    def forward(self, vol, prc):
        x = torch.cat((vol, prc), dim=2)
        x = torch.flatten(x, 1, 2)
        x = self.linear(x)
        return x


class LSTM(nn.Module): # 不加邻接图
    ...
    def forward(self, vol, prc):  # vol.shape = [batch, node, seq]
        x = torch.stack([vol, prc], dim=3)
        x = self.encoder(x)
...
class LstmGcn(nn.Module):
    ...
    def forward(self, vol, prc):  # vol.shape = [batch, node, seq]
        x = torch.stack([vol, prc], dim=3) # TODO
        x = self.encoder(x)
        x = torch.squeeze(x)
        #  l1
        x = self.gcn_l1(x)
...
class LstmGat(nn.Module):
    ...
    def forward(self, vol, prc, occ, day, fri, hor, wea):  # vol.shape = [batch, node, seq]
        x = torch.stack([vol, prc ,occ, day, fri, hor, wea], dim=3)
        x = self.encoder(x)
        x = torch.squeeze(x)

        # first layer
        atts_mat = self.gat_l1(x)  # attention matrix, dense(nodes, nodes)
        vol_conv1 = torch.matmul(atts_mat, x)  # (b, n, s)
        vol_conv1 = self.dropout(self.LeakyReLU(self.gcn(vol_conv1)))

        # second layer
        atts_mat2 = self.gat_l2(vol_conv1)  # attention matrix, dense(nodes, nodes)
        vol_conv2 = torch.matmul(atts_mat2, vol_conv1)  # (b, n, s)
        vol_conv2 = self.dropout(self.LeakyReLU(self.gcn(vol_conv2)))

        # lstm
        x = vol_conv2.transpose(1, 2)
        x, _ = self.lstm(x)
        x = x.transpose(1, 2) # revised x,
...
class HSTGCN(nn.Module):
    def __init__(self, seq, n_fea, adj_distance, adj_demand, alpha=0.5):
        ...
        self.gru1 = nn.LSTM(self.nodes, self.nodes, num_layers=2, batch_first=True)
        self.demand_gcn_l1 = nn.Linear(hidden, hidden)
        self.demand_gcn_l2 = nn.Linear(hidden, hidden)
        self.gru2 = nn.LSTM(self.nodes, self.nodes, num_layers=2, batch_first=True)
